...
class PackageImportTest(unittest.TestCase):
    ...
    def test_ninja_client_can_be_imported_as_plugin_package(self):
        package_root = Path(__file__).resolve().parents[1]
        package_parent = Path(__file__).resolve().parents[2]
        original_path = list(sys.path)
        original_modules = {
        ...
        try:
            sys.path = [entry for entry in sys.path if entry not in {"", str(package_root)}]
            sys.path.insert(0, str(package_parent))
            module = importlib.import_module("astrbot_plugin_poe2_price.services.ninja_client")
        finally:
            sys.path = original_path
            sys.modules.update(original_modules)

        self.assertTrue(hasattr(module, "NinjaClient"))
